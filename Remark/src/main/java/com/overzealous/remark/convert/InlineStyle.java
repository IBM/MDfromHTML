...
/**
 * Handles various inline styling (italics and bold), such as em, i, strong, b,
 * span, and font tags.
 * 
...
public class InlineStyle extends AbstractNodeHandler {
   ...
   private static final Pattern ITALICS_PATTERN = Pattern
      .compile("font-style:\\s*italic", Pattern.CASE_INSENSITIVE);
   private static final Pattern BOLD_PATTERN = Pattern
      .compile("font-weight:\\s*bold", Pattern.CASE_INSENSITIVE);

   private static final Pattern INWORD_CHARACTER = Pattern.compile("\\w");

   private static final Pattern SPACE_CONTENT_SPACE = Pattern
   ...
   /**
    * Renders inline styling (bold, italics) for the given tag. It handles
    * implicit styling ({@code em}, {@code strong}) as well as explicit styling
    * via the {@code style} attribute.
    * <p>
    * This object keeps track of the depth of the styling, to prevent recursive
    ...
    *
    * <p>
    * A naive method would be render the example incorrectly (the output would
    * be {@code *hello **world*})
    * </p>
    *
    * @param parent
   ...
   public Node handleNode(NodeHandler parent, Element node,
      ...
      Node result = null;
      // special handling for inline span
      ...
      if (checkInnerBlock(node, pw, level)) {
         ...
         Rules rules = checkInword(node, converter, pw, level);
         if (rules.emphasisPreserved) {
            checkTag(node, rules, pw, level);

            if (rules.bold || rules.italics) {
               result = handleStyled(parent, node, converter, rules, pw, baseUri, domain,
                  level, searchLevel);
            } else {
               result = converter.walkNodes(this, node, converter.inlineNodes,
                  pw, baseUri, domain, level, searchLevel);
            }
         } else { // emphasis has been disabled for this section
            // mark as if emphasis was already processed
            italicDepth++;
            boldDepth++;
            result = converter.walkNodes(this, node, converter.inlineNodes, pw,
    ...
    * 
    * <p>
    * It's unique because we have to deal with leading and trailing spaces,
    * among other issues.
    * </p>
    * 
    * @param parent
    ...
    *           Parent converter for this object.
    * @param rules
    *           The styling rules that are active
    * @param pw
    *           Provenance Writer to receive provenance annotations mapping
    *           generated markdown to document element(s)
    * @param baseUri
    ...
    * @return the Node where level matches searchLevel, otherwise, returns null
    */
   private Node handleStyled(NodeHandler parent, Element node,
      DocumentConverter converter, Rules rules, ProvenanceWriter pw,
      ...
      String content = converter.getInlineContent(this, node, true, pw, baseUri, domain,
         level, searchLevel, nodeSet);
      if (nodeSet.size() > 0) {
      ...
      if (content.length() > 0) {
         ...
         if (parts.find()) {
            ...
            if (parts.group(2).length() > 0) {
               ...
               md += start(rules, parts.group(1), converter, pw, level);
               ...
               md += end(rules, parts.group(3), converter, pw, level);
            }
    ...
    * @param level
    * @return True is there is a block inside this node (which would be invalid
    *         HTML)
    */
   private boolean checkInnerBlock(Element node, ProvenanceWriter aw,
      String level) {
    ...
    * situation where a styled node is immediately <em>followed</em> by a text
    * node, and that text node starts with a word character.
    * </p>
    *
    * @param node
    *           The current node (should be an inline-styled node)
    * @param converter
   ...
   private Rules checkInword(Element node, DocumentConverter converter,
      ProvenanceWriter aw, String level) {
      ...
      if (!iwe.isEmphasisPreserved() || iwe.isAdditionalSpacingNeeded()) {
         ...
         if (n != null && n instanceof TextNode) {
            ...
            String text = tn.text();
            if (INWORD_CHARACTER.matcher(text.substring(text.length() - 1))
               .matches()) {
    ...
    *           The node to look at
    * @param rules
    *           The rules object to hold the result
    * @param aw
    *           Annotation Writer to receive annotations mapping generated
    *           markdown to document element(s)
    * @param level
    */
   private void checkTag(Element node, Rules rules, ProvenanceWriter aw,
      String level) {
    ...
    *           markdown to document element(s)
    * @param level
    * @return markdown for starting inlined content
    */
   private String start(Rules style, String leadingSpaces,
      DocumentConverter converter, ProvenanceWriter aw, String level) {
      String md = "";
      if (style.addSpacing && (italicDepth == 0 || boldDepth == 0)
    ...
    *           markdown to document element(s)
    * @param level
    * @return markdown generated for ending inlined content
    */
   private String end(Rules style, String trailingSpaces,
      DocumentConverter converter, ProvenanceWriter aw, String level) {
      String md = "";
      if (style.bold) {
         if (boldDepth == 0) {
