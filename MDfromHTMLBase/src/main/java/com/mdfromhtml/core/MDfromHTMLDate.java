...
/**
 * Special MDfromHTML Date object extends java.util.Date with special constructors
 * takingMDfromHTML specific date formatted strings. The toString implementation
 * provides a formatted date suitable for sorting in the format
 * "yyyy/MM/dd-HH:mm:ss.SSS(Z)"
 */
public class MDfromHTMLDate extends Date implements Cloneable {
   ...
   static public String m_strClassName = MDfromHTMLUtils
      .getNameFromClass(MDfromHTMLDate.class);
    ...
    */
   static public long elapsedTime(MDfromHTMLDate dateStart, MDfromHTMLDate dateEnd) {
   ...
   static public String[] getShortTimeZoneList(String strPrefix) {
      ...
      for (int i = 0; i < output.length; i++) {
         ...
         output[i] = output[i] + " " + MDfromHTMLUtils
            .arrayListToListString(mapTZValues.get(vectKeys.elementAt(i)));
      }
      return output;
   }
   ...
   /**
    * @param strPrefix
    *           the prefix to be compared against the timezone ID's as a filter.
    *           If the timezone id starts with the supplied prefix it will be
    *           included in the returned list. If the prefix is null or empty
   ...
   static public String[] getTimeZoneList(String strPrefix) {
      ...
      for (int i = 0; i < timezones.length; i++) {
         ...
         outputList
            .add(MDfromHTMLUtils.convertMillisecondsToTimeZone(tz.getRawOffset())
               + " " + timezones[i]);
      }
   ...
   /**
    * Tests for equality to the UNDEFINED_MDfromHTMLDate which is one millisecond after
    * the epoch date (midnight 1/1/1970 GMT). This is used to identify and/or
    * initialize anMDfromHTMLDate value that has yet to receive a valid value.
   ...
   static public void main(String[] args) {
      System.out.println(
         "Undefined Date is: " + UNDEFINED_MDfromHTMLDate.toStringDBTimestamp());
      ...
      while (true) {
         ...
         try {
            ...
            System.out.println(date.toString(MDfromHTMLDate.CREATE_DATE_FORMAT_11,"GMT"));
            System.out.println(date.toString(MDfromHTMLDate.CREATE_DATE_FORMAT_12,"GMT"));
   ...
   static synchronized public String makeFormattedDate(Date date,
      ...
      int iTZMillisecs = MDfromHTMLUtils.convertTimeZoneToMilliseconds(strTimeZone);
   ...
   /**
    * Converts the inputMDfromHTMLDate to an undefinedMDfromHTMLDate if the inputMDfromHTMLDate is
    * null.
    * 
    * @param date
    *           MDfromHTMLDate to be tested against null and converted.
    * @return An undefinedMDfromHTMLDate if the inputMDfromHTMLDate was null, otherwise, the
    *         inputMDfromHTMLDate is echoed back.
    */
    ...
    */
   public MDfromHTMLDate(int iYear, int iMonth, int iDay, int iHours, int iMinutes,
    ...
    * 
    * @param strDate
    *           theMDfromHTML formatted date string to be transformed into theMDfromHTMLDate
    *           object.
    * @see #CREATE_DATE_FORMAT
    * @see #CREATE_DATE_FORMAT_0
    * @see #CREATE_DATE_FORMAT_1
    ...
    * this date. If no end date has been set, then the duration is 0L. If no
    * start date has been set, then the duration is the number of milliseconds
    * since the epoch (midnight 1/1/70 0000) until the end date. If the start
    * date is greater than the end date, then 0L is returned.
    * 
    * @param dateFrom
    *           the date from which the elapsed time is calculated (e.g., until
    *           this date).
   ...
   /**
    * Tests for equality to the UNDEFINED_MDfromHTMLDate which is one second after the
